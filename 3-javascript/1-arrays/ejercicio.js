...
function vaciarCarritoConTotal() {
    ...
    console.log('articulos actuales:', carrito);
...
function cuentaDeAlumnos() {
    ...
    let grupoNuevo = grupoA.concat(grupoB);
    console.log('El nuevo grupo tiene los siguientes alumnos', grupoNuevo);
}
console.log(cuentaDeAlumnos());
/*
...
function transmitiendoBatch() {
    ...
    for (let i = 0; i < backup.length; i += tamaLote) {
        const lote = backup.slice(i, i + tamaLote);
        noLote++;
        console.log(lote, lote.length);
    }

    console.log(`Total de lotes: ${noLote}`);
}

console.log(transmitiendoBatch());
